    ...
    useEffect(() => {
      ...
      (async () => {
        try {
          ...
          setUsers(mapped);
          if (mapped[0]) setFlt(p => ({ ...p, userId: mapped[0].id, walletId: "" }));
        } catch {/* ignore */}
      })();
      return () => { alive = false; };
    }, []);
      ...
      <div className="space-y-6 min-h-screen">
        ...
        {summary && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <StatCard title={t("dashboard.kpis.progress")}     value={`${Math.round((summary?.pctIncomeReceived ?? 0) * 100)}% / ${Math.round((summary?.pctExpensePaid ?? 0) * 100)}%`} />
            <StatCard title={t("dashboard.kpis.walletBalance")} value={fmtCurrency((summary?.totalIncome ?? 0)-(summary?.totalExpense ?? 0), currency)} />
          </div>
        )}
